...
async def get_exam_schedules(term: str = None, current_user: TokenData = Depends(get_current_user)):
    """
    試験時間割一覧を取得
    ...
    try:
        ...
        schedules = user_data.get("exam_schedules_by_term", {}).get(selected_term, [])
...
async def create_exam_schedule(
    ...
    try:
        ...
        user_data = load_user_data(user_id)
        selected_term = normalize_term_key(term)
        user_data.setdefault("exam_schedules_by_term", {}).setdefault(selected_term, [])
        schedules = user_data["exam_schedules_by_term"][selected_term]
